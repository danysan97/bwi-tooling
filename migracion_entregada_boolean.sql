-- Migracion: entregada pasa de estado a boolean
-- Ejecutar en Supabase SQL Editor

-- 1. Agregar columna entregada
ALTER TABLE ordenes_trabajo ADD COLUMN entregada BOOLEAN DEFAULT FALSE;

-- 2. Migrar órdenes que tenían estado 'entregada' → terminada + entregada=true
UPDATE ordenes_trabajo SET estado = 'terminada', entregada = TRUE WHERE estado = 'entregada';

-- 3. Recrear vista_ordenes con la columna entregada
...
SELECT
  ...
  (SELECT string_agg(s2.comentarios, ' | ' ORDER BY s2.fecha_registro)
   ...
   WHERE s2.orden_id = o.no_orden AND s2.comentarios IS NOT NULL AND s2.comentarios != '') AS comentarios,
